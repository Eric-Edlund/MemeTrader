...
//        final var now = OffsetDateTime.now().truncatedTo(ChronoUnit.DAYS);
//        for (int id : stockIds) {
//            final var name = memeStockRepository.getMetadata(id).title();
//            System.out.println(now);
//            System.out.println(now.minusWeeks(1));
//            memeStockService.getHistory(id, now.minusWeeks(1), now).forEach((point) -> {
//                System.out.println(point);
//            });
//            final var lastWeek = memeStockService.getHistory(id, now.minusWeeks(1), now.minusDays(6)).get(0).price();
//
//            final var yesterday = memeStockService.getHistory(id, now.minusDays(1), now).get(0).price();
//            final var today = memeStockService.getHistory(id, now, now.plusDays(1)).get(0).price();
//
...
//    }
    ...
    private record ChatGPTResponse(List<ArticleGenerator.Choice> choices) {};
    private record Choice(ArticleGenerator.Message message) {};
    private record Message(String role, String content) {};

    private List<String> extractTitleAndBody(String response) {
        ObjectMapper mapper = new ObjectMapper().configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        try {
            final var result = mapper.readValue(response, ArticleGenerator.ChatGPTResponse.class);
